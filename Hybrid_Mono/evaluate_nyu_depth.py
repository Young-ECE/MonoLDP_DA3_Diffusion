...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        scalenet_path = os.path.join(opt.load_weights_folder, "scalenet.pth")
        regression_path = os.path.join(opt.load_weights_folder, "regression.pth")
        ...
        regression_heads = nn.ModuleList([
            networks.ProbabilisticScaleRegressionHead(in_channels=ch) for ch in encoder.num_ch_enc
        ])
        ...
        global_depth = np.concatenate(global_depth)
    ...
    for i in range(pred_disps.shape[0]):
        ...
        mask = np.logical_and(gt_depth > MIN_DEPTH, gt_depth < global_depth[i])#MAX_DEPTH)
        ...
        mask_pred_depth[mask_pred_depth > global_depth[i]] = global_depth[i]#MAX_DEPTH        
