...
class DepthAnythingV3Wrapper(nn.Module):
    ...
    def _load_da3_model(self):
        ...
        try:
            ...
            # Helper function to find snapshot directory in HuggingFace cache
            def find_snapshot_dir(cache_root):
                """Find the actual snapshot directory containing model files"""
                hf_model_name = f"depth-anything/{self.model_name}"
                model_cache_name = f"models--{hf_model_name.replace('/', '--')}"
                model_cache_path = os.path.join(cache_root, model_cache_name)
            ...
            # If model_path is provided, try to use it
            model_load_path = None
            if self.model_path and os.path.exists(self.model_path):
                # Check if it's a direct snapshot directory (contains model.safetensors)
                if os.path.exists(os.path.join(self.model_path, "model.safetensors")):
                    model_load_path = self.model_path
                    print(f"  Loading from snapshot directory: {model_load_path}")
                # Check if it's a HuggingFace cache root directory
                elif find_snapshot_dir(self.model_path):
                    model_load_path = find_snapshot_dir(self.model_path)
                    print(f"  Found snapshot in cache: {model_load_path}")
                # Check if it's a model cache directory (models--*)
                elif "models--" in os.path.basename(self.model_path):
                    # Try to find snapshot in this directory
                    snapshots_path = os.path.join(self.model_path, "snapshots")
                    if os.path.exists(snapshots_path):
                        snapshots = [d for d in os.listdir(snapshots_path) 
                                   if os.path.isdir(os.path.join(snapshots_path, d))]
                        if snapshots:
                            snapshot_dir = os.path.join(snapshots_path, snapshots[0])
                            if os.path.exists(os.path.join(snapshot_dir, "model.safetensors")):
                                model_load_path = snapshot_dir
                                print(f"  Found snapshot in model cache: {model_load_path}")
            
            # If no valid path found, try to find in cache_dir
            if model_load_path is None:
                snapshot_dir = find_snapshot_dir(self.cache_dir)
                if snapshot_dir:
                    model_load_path = snapshot_dir
                    print(f"  Found cached model: {model_load_path}")
            ...
            if model_load_path:
                ...
                if self.model_path:
                    print(f"  (Using cache directory: {self.model_path})")
                    # Set cache_dir to the provided path
                    actual_cache_dir = self.model_path
                else:
                    actual_cache_dir = self.cache_dir
