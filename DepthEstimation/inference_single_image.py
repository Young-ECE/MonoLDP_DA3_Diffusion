...
def prepare_model_for_test(args, device):
    ...
    scalenet_path = os.path.join(model_path, "scalenet.pth")
    regression_path = os.path.join(model_path, "regression.pth")
    ...
    scalenet = scalenet.to(device).eval()
    regression_heads.to(device).eval()
    
    return encoder, decoder, scalenet, regression_heads, encoder_dict['height'], encoder_dict['width']

def inference(args):
    device = torch.device("cpu")
    encoder, decoder, scalenet, regression_heads, thisH, thisW = prepare_model_for_test(args, device)
    ...
    with torch.no_grad():
        ...
        depth_factors = scalenet(features_tmp)
        ...
        scaled_disp, _ = disp_to_depth(disp, 0.1, max_depth)
